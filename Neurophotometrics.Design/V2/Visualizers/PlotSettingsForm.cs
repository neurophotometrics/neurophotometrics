...
namespace Neurophotometrics.Design.V2.Visualizers
{
    public partial class PlotSettingsForm : Form
    {
        ...
        private void AddColumns()
        {
            ...
            foreach (var signalName in PlotSettings[0].SignalSettings.Select(signalSetting => signalSetting.Name))
            {
        ...
        private void AddRows()
        {
            ...
            for (var i = 0; i < Top_DataGridView.Rows.Count; i++)
            {
                ...
                PlotSettings[i].SignalSettings = PlotSettings[i].SignalSettings.OrderBy(setting => setting.Name).ToList();
        ...
        private string GetSignal(int columnIndex)
        {
            ...
            return PlotSettings[0].SignalSettings[signalIndex].Name;
        }
        ...
        private void DrawSignalColumns(PaintEventArgs e)
        {
            ...
            for (var i = 0; i < dispRects.Length; i++)
            {
                var parentColName = signalSettings[i].Name;
        ...
        private int GetDisplayRectIndex(DataGridViewColumn dataGridViewColumn)
        {
            ...
            for (var i = 0; i < signalSettings.Count; i++)
                if (dataGridViewColumn.Name.Contains(signalSettings[i].Name))
                    return i;
            return -1;
        }
