...
namespace Neurophotometrics.Design.V1.Visualizers
{
    public partial class ActivityView : UserControl
    {
        ...
        internal void TryUpdateNewFrame(PhotometryDataFrame dataFrame)
        {
            try
            {
                UpdateNewFrame(dataFrame);
            }
            catch (Exception ex)
            {
        ...
        private void UpdateNewFrame(PhotometryDataFrame dataFrame)
        {
        ...
        private void EnsurePlotTable(PhotometryDataFrame dataFrame)
        {
        ...
        private void EnsurePlotQuantity(PhotometryDataFrame dataFrame)
        {
        ...
        {
            var numVisiblePlots = VisualizerSettings.PlotSettings.Count(plotSeeting => plotSeeting.IsVisible);
        ...
        {
            if (dataFrame.Flags == FrameFlags.None)
                return;
            var ledFlag = (FrameFlags)(ushort)((ushort)dataFrame.Flags & 0x7);
        ...
        private void UpdateSignal(string regionName, string signalName, string settingName, object settingValue)
        {
            ...
            var signalSetting = plotSetting.SignalSettings.First(signal => GetSignalName(signal.LEDFlag) == signalName);
    ...
    public class RegionPlotControl : ZedGraphControl
    {
        ...
        {
            var filteredSignalPanes = SignalPanes.Where(pane => GetName(((FrameFlags)pane.Tag)) == led);
            if (!filteredSignalPanes.Any()) return;
            var signalPane = filteredSignalPanes.First();
            signalPane.UpdateAutoScale(isAutoScale);
        }

        private string GetName(FrameFlags ledFlag)
        {
            switch (ledFlag)
            {
                case FrameFlags.None:
    ...
    public class SignalPane : GraphPane
    {
        ...
        public SignalPane(PhotometryRegion photometryRegion, SignalSetting signalSetting, int capacity)
        {
            ...
            IsAutoScale = signalSetting.Scaling.Mode == AutoScalingMode.Auto;
            ...
            YAxisList.Clear();
            var axisName = GetAxisName(signalSetting.LEDFlag);
            AddYAxis(axisName);
            ...
            YAxis.Title.FontSpec.Size = axisName.Length <= 4 ? 16f : 12f;
