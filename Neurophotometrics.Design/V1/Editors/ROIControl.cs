...
namespace Neurophotometrics.Design.V1.Editors
{
    public partial class ROIControl : UserControl
    {
        ...
        internal void TryUpdateKeyDown(Keys key)
        {
            try
            {
                UpdateKeyDown(key);
        ...
        private void UpdateKeyDown(Keys key)
        {
            // Do nothing if IsMouseDown
            if (IsMouseDown)
                return;

            if (key == Keys.PageUp && ImageScale < MaxImageScale) ImageScale = (byte)Math.Min(ImageScale + ScaleIncrement, MaxImageScale);
            else if (key == Keys.PageDown && ImageScale > MinImageScale) ImageScale = (byte)Math.Max(ImageScale - ScaleIncrement, MinImageScale);
            else if (key == Keys.Delete) DeleteROI();
        }
