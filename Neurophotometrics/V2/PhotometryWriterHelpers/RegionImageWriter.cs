...
namespace Neurophotometrics.V2.PhotometryWriterHelpers
{
    public class RegionImageWriter
    {
        ...
        private void StoreFirstRegionImage(PhotometryDataFrame dataFrame)
        {
            ...
            FirstRegionImage = GetImage(dataFrame);
        ...
        private void StoreCurrentRegionImage(PhotometryDataFrame dataFrame)
        {
            ...
            CurrentRegionImage = GetImage(dataFrame);
            DrawRegions(dataFrame, CurrentRegionImage);
        }

        private Bitmap GetImage(PhotometryDataFrame dataFrame)
        {
            var imgWidth = (int)dataFrame.PhotometryImage.Width;
            var imgHeight = (int)dataFrame.PhotometryImage.Height;

            var outImg = new Bitmap(imgWidth, imgHeight, PixelFormat.Format24bppRgb);
            var outImg_bitmapData = outImg.LockBits(new Rectangle(0, 0, imgWidth, imgHeight), ImageLockMode.WriteOnly, outImg.PixelFormat);
            var inImg_bitmapData = dataFrame.PhotometryImage.Bitmap.LockBits(new Rectangle(0, 0, imgWidth, imgHeight), ImageLockMode.ReadOnly, dataFrame.PhotometryImage.Bitmap.PixelFormat);
            unsafe
            {
                ...
                {
                    var row_offsetInPixels = imgWidth * y;

                    var outImg_ptrCurrentPixel = outImg_ptrFirstPixel + row_offsetInPixels * DisplayBPP;
                    var inImg_ptrCurrentPixel = inImg_ptrFirstPixel + row_offsetInPixels;
                    var inImg_ptrLastPixel = inImg_ptrCurrentPixel + imgWidth;

                    while (inImg_ptrCurrentPixel < inImg_ptrLastPixel)
                    {
                        var pixelValue = (byte)Math.Min(*(inImg_ptrCurrentPixel++) * ImageScale, byte.MaxValue);
                        outImg_ptrCurrentPixel[0] = pixelValue;
                        outImg_ptrCurrentPixel[1] = pixelValue;
                        outImg_ptrCurrentPixel[2] = pixelValue;
                        outImg_ptrCurrentPixel += DisplayBPP;
                    }
                });
            }
            dataFrame.PhotometryImage.Bitmap.UnlockBits(inImg_bitmapData);
