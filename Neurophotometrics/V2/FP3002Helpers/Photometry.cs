...
namespace Neurophotometrics.V2.FP3002Helpers
{
    internal class Photometry : Combinator<PhotometryImage, PhotometryDataFrame>
    {
        ...
        {
            if (photometryImage.BitsPerPixel == 8)
                return GetMono8Activity(photometryImage);
            else
                return new RegionActivity[1];
        }

        private RegionActivity[] GetMono8Activity(PhotometryImage photometryImage)
        {
            var cropInPixels = new Rectangle(0, 0, (int)photometryImage.Width, (int)photometryImage.Height);
            ...
            unsafe
            {
                var img_bitmapData = photometryImage.Bitmap.LockBits(new Rectangle(0, 0, photometryImage.Width, photometryImage.Height), ImageLockMode.ReadOnly, PixelFormat.Format8bppIndexed);
                ...
                for (var i = 0; i < regions.Count; i++)
                {
                    ...
                    var reg_widthInBytes = reg_widthInPixels;
                    var reg_offsetInBytes = Math.Max(0, region.Rectangle.Y - photometryImage.OffsetY) * photometryImage.Width + Math.Max(0, region.Rectangle.X - photometryImage.OffsetX);
                    var rowSums = new double[reg_heightInPixels];

                    var reg_ptrFirstPixel = img_ptrFirstPixel + reg_offsetInBytes;
                    Parallel.For(0, reg_heightInPixels, y =>
                    {
                        var row_offsetInBytes = photometryImage.Width * y;
                        var row_ptrFirstPixel = reg_ptrFirstPixel + row_offsetInBytes;
                        var row_ptrLastPixel = row_ptrFirstPixel + reg_widthInBytes;
                    ...
                    var normalPixelValue = avgPixelValue / byte.MaxValue;
